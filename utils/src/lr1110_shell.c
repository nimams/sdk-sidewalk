...
static void cmd_gnss_autonomus_work(struct k_work *item)
{
   ...
   lr11xx_status_t status = lr11xx_gnss_apply_mixer_cfg_workaround(drv_ctx);
   if (status == LR11XX_STATUS_ERROR)
		shell_error(gnss_work->shell, "assisted scan fail");
   else
      shell_info(gnss_work->shell, "called lr11xx_gnss_apply_mixer_cfg_workaround");
...
static void cmd_gnss_assisted_work(struct k_work *item)
{
   ...
   status = lr11xx_gnss_apply_mixer_cfg_workaround(drv_ctx);
   if (status == LR11XX_STATUS_ERROR)
		shell_error(gnss_work->shell, "assisted scan fail");
   else
      shell_info(gnss_work->shell, "called lr11xx_gnss_apply_mixer_cfg_workaround");
